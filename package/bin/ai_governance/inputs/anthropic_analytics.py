...
def _collect(logger, session_key, input_key, input_item, event_writer) -> int:
    ...
    starting = ending - timedelta(days=lookback_days - 1)
    ...
    for flag, fetch, sourcetype, category in collectors:
        ...
        try:
            ...
            logger.warning("Anthropic analytics %s collection failed: %s", category, exc)

    checkpoint.update(ckpt_key, last_ending_date=ending_date)
