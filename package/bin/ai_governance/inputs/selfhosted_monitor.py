...
def _collect(logger, session_key, input_key, input_item, event_writer) -> int:
    ...
    if models is not None:
        count += _process_inventory(
            logger, models, api, state, checkpoint, ckpt_key,
            account_name, index, source, event_writer, now_iso,
        )
...
def _process_inventory(
    ...
    if known_ids is not None:
        ...
        if added or removed:
            logger.info(
                "Model inventory changed: +%s -%s", len(added), len(removed)
            )
    ...
    if (
        ...
        for model in models:
            payload = envelope(
                dict(model, snapshot_time=now_iso, server_type=api.server_type),
                provider=PROVIDER_SELFHOSTED,
                category="directory",
